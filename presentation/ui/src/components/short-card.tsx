...
export function ShortCard({ short }: ShortCardProps) {
  ...
  if (short.videoUrl.includes('vsMydMDi3rI')) {
    console.log('ShortCard Debug:', { url: short.videoUrl, videoId, isYouTube });
  }
        ...
        <CardContent className="p-0">
          <div className="relative aspect-video overflow-hidden rounded-lg bg-black">
            <Image
              ...
              className={`object-cover w-full h-full transition-opacity duration-300 group-hover:scale-105 ${isHovering && !isYouTube ? 'opacity-0' : 'opacity-100'}`}
            />
            {!isYouTube && (
              <video
                ref={videoRef}
